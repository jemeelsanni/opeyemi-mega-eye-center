...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      {/* Sidebar */}
      <aside 
        className={`fixed inset-y-0 left-0 z-30 w-64 bg-[#1a1a2e] text-white transform ${
      ...
      >
        <div className="flex items-center justify-between h-16 px-6 border-b border-[#16213e]">
          <Link to="/admin" className="flex items-center space-x-2">
            <span className="text-[#FFA500] text-xl font-bold">Eye Center Admin</span>
          </Link>
          <button 
            onClick={() => setSidebarOpen(false)} 
            ...
            {filteredMenuItems.map((item) => (
              <div key={item.name}>
                {item.hasSubmenu ? (
                  <div>
                    <button
                      onClick={() => setBlogSubmenu(!blogSubmenu)}
                      className={`flex items-center justify-between w-full px-4 py-3 text-sm rounded-md ${
                        isBlogSectionActive() ? 'bg-[#16213e] text-[#FFA500]' : 'hover:bg-[#16213e]'
                      }`}
                    >
                        ...
                        {item.submenuItems!.map((subItem) => (
                          <Link
                            ...
                            className={`block px-4 py-2 text-sm rounded-md ${
                              isActive(subItem.path) ? 'bg-[#16213e] text-[#FFA500]' : 'hover:bg-[#16213e]'
                            }`}
                          >
                            {subItem.name}
                          </Link>
                  ...
                  <Link
                    ...
                    className={`flex items-center px-4 py-3 text-sm rounded-md ${
                      isActive(item.path) ? 'bg-[#16213e] text-[#FFA500]' : 'hover:bg-[#16213e]'
                    }`}
                  >
                    <span className="mr-3">{item.icon}</span>
                    <span>{item.name}</span>
          ...
          <div className="px-4 py-4 border-t border-[#16213e] mt-auto lg:hidden">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <FaUserCircle className="h-9 w-9 text-[#FFA500]" />
              </div>
              <div className="ml-3">
                ...
                <div className="absolute right-0 z-10 w-80 mt-2 bg-white rounded-lg shadow-lg origin-top-right">
                  ...
                  {notifications.length > 0 && (
                    <div className="p-2 text-center border-t border-gray-100">
                      <button className="text-sm text-[#FFA500] hover:text-[#FF9000]">
                        Mark all as read
                      </button>
                    </div>
                  )}
